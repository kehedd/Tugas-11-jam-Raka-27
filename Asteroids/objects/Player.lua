    ...
    return {
        ...
        draw = function (self, faded)
            ...
            if not self.exploading then
                ...
                -- change opacity depending if player is invincible_seen
                if self.invincible_seen then
                ...
                love.graphics.polygon(
                    ...
                    self.x + ((4 / 3) * self.radius) * math.cos(self.angle),
                    self.y -  ((4 / 3) * self.radius) * math.sin(self.angle),
                    self.x - self.radius * (0.5 * math.cos(self.angle) + 0.5 * math.sin(self.angle)),
                    self.y + self.radius * (0.5 * math.sin(self.angle) - 0.5 * math.cos(self.angle)),
                    self.x - self.radius * (1.5 * math.cos(self.angle) + 0 * math.sin(self.angle)),
                    self.y + self.radius * (1.5 * math.sin(self.angle) - 0 * math.cos(self.angle)),
                    self.x - self.radius * (0.5 * math.cos(self.angle) - 0.5 * math.sin(self.angle)),
                    self.y + self.radius * (0.5 * math.sin(self.angle) + 0.5 * math.cos(self.angle))
                )
        ...
        drawLives = function (self, faded)
            ...
            for i = 1, self.lives do
                ...
                love.graphics.polygon(
                    ...
                    (i * x_pos) - self.radius * (0.5 * math.cos(VIEW_ANGLE) + 0.5 * math.sin(VIEW_ANGLE)),
                    y_pos + self.radius * (0.5 * math.sin(VIEW_ANGLE) - 0.5 * math.cos(VIEW_ANGLE)),
                    (i * x_pos) - self.radius * (1.5 * math.cos(VIEW_ANGLE) + 0 * math.sin(VIEW_ANGLE)),
                    y_pos + self.radius * (1.5 * math.sin(VIEW_ANGLE) - 0 * math.cos(VIEW_ANGLE)),
                    (i * x_pos) - self.radius * (0.5 * math.cos(VIEW_ANGLE) - 0.5 * math.sin(VIEW_ANGLE)),
                    y_pos + self.radius * (0.5 * math.sin(VIEW_ANGLE) + 0.5 * math.cos(VIEW_ANGLE))
                )
            end
