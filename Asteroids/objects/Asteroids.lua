...
function Asteroids(x, y, ast_size, level, sfx)
    local ASTEROID_VERT = 5 -- Number of points for the star
    local ASTEROID_JAG = 0.4
    local ASTEROID_SPEED = math.random(50) + (level * 2)

    local vert = ASTEROID_VERT
    local offset = {}
    for i = 1, vert + 1 do
    ...
    return {
        ...
        draw = function (self, faded)
            ...
            for i = 0, self.vert - 1 do
                local angle1 = (i * 2 * math.pi / self.vert) - math.pi / 2
                local angle2 = ((i + 0.5) * 2 * math.pi / self.vert) - math.pi / 2
                table.insert(points, self.x + self.radius * math.cos(angle1))
                table.insert(points, self.y + self.radius * math.sin(angle1))
                table.insert(points, self.x + self.radius * 0.5 * math.cos(angle2))
                table.insert(points, self.y + self.radius * 0.5 * math.sin(angle2))
            end
